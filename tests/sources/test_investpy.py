...
from bearish.sources.investpy import InvestPySource, RAW_EQUITIES_DATA_URL, RAW_CRYPTO_DATA_URL, RAW_ETF_DATA_URL
...
def test_read_assets() -> None:
    with requests_mock.Mocker() as req:
        req.get(
            ...
            text=Path(__file__).parents[1].joinpath("data/sources/investpy/equities.csv").read_text(),
        )
        req.get(
            RAW_CRYPTO_DATA_URL,
            text=Path(__file__).parents[1].joinpath("data/sources/investpy/cryptos.csv").read_text(),
        )
        req.get(
            RAW_ETF_DATA_URL,
            text=Path(__file__).parents[1].joinpath("data/sources/investpy/etfs.csv").read_text(),
        )
